...
const SearchForm = ({ value, onChange }) => (
  <form className="flex justify-center gap-[2em] mt-24">
    <div className={s.inputBlock}>
      <input
        value={value}
        className="text-[#fafafa] font-medium text-[1.1em] w-full bg-white bg-opacity-0"
        onChange={onChange}
        placeholder="Search..."
      />
    </div>
...
const ItemCard = ({ item }) => (
  <Link to={'/films/' + item.id}>
    <div className='relative py-28 h-[88.5%] rounded-xl overflow-hidden mt-12'>
      <div>
      </div>
      <div className='absolute bottom-0 right-0 h-60 flex items-end w-full bg-gradient-to-t from-black to-transparent opacity-80 py-1 rounded-md'>
        <div className='flex flex-col w-full h-full'>
          <div className='w-full h-full bg-cover bg-center' style={{ backgroundImage: `url("https://image.tmdb.org/t/p/w500${item.poster_path}")` }}></div>
